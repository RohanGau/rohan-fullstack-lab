...
export function useSlotBooking() {
  ...
  const endDateTime = useMemo(() => (startDateTime ? addMinutes(startDateTime, duration) : undefined), [startDateTime, duration]);
  ...
  const submit = async () => {
    ...
    try {
      ...
      const payload = { name, email, message, date: startDateTime.toISOString(), duration, status: "booked", captchaToken };
      await apiFetch("/api/slots", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify(payload) });
      setSuccess(true);
    } catch (e: any) {
